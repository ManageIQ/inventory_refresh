class ExtManagementSystem < ::ActiveRecord::Base
  ...
  has_many :vms,               -> { active }, :foreign_key => :ems_id, :dependent => :destroy
  has_many :miq_templates,     -> { active }, :foreign_key => :ems_id, :dependent => :destroy
  has_many :vms_and_templates, -> { active }, :foreign_key => :ems_id, :class_name => "VmOrTemplate"
  has_many :hardwares,         :through => :vms_and_templates
